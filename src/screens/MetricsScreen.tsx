...
const SLICE_COLORS: Record<EmotionCategory, string> = {
  calm: "#A9D5F0",
  happy: "#F6C453",
  sad: "#90CAF9",
  mad: "#F28B82",
...
export default function MetricsScreen() {
  ...
  const [counts, setCounts] = useState<Record<EmotionCategory, number>>(
    Object.fromEntries(EMOTION_CATEGORIES.map((c) => [c, 0])) as Record<EmotionCategory, number>
  );
  ...
  // Refresh when the screen/tab gains focus
  useFocusEffect(
    useCallback(() => {
      fetchCounts();
      return undefined;
  ...
  // Also recompute when the range changes (buttons)
  useEffect(() => {
    fetchCounts();
  }, [fetchCounts]);
                ...
                {EMOTION_CATEGORIES.filter((c) => (counts[c] || 0) > 0).map((cat) => {
                  ...
                  return (
                    <View key={cat} style={styles.legendRow}>
                      <View style={[styles.legendChip, { backgroundColor: SLICE_COLORS[cat] }]} />
...
const styles = StyleSheet.create({
  ...
  rangeBtnActive: { borderColor: theme.text },
  rangeText: { color: theme.textSubtle },
  rangeTextActive: { fontWeight: "700", color: theme.text },
  ...
  legend: { flex: 1 },
  total: { fontWeight: "700", marginBottom: 8, color: theme.text },
  legendRow: { flexDirection: "row", alignItems: "center", marginBottom: 6 },
  legendChip: { width: 14, height: 14, borderRadius: 7, marginRight: 8 },
  legendLabel: { flex: 1, color: theme.text },
  legendValue: { color: theme.textSubtle },

  empty: { textAlign: "center", color: theme.textSubtle, padding: 16 },
});
