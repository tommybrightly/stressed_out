...
export default function MetricsScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const now = Date.now();
      const cutoff = range === "7d" ? now - 7 * 86400_000 : range === "30d" ? now - 30 * 86400_000 : 0;
      const filtered = cutoff ? all.filter((e) => e.at >= cutoff) : all;
      setCounts(countsFrom(filtered));
    })();
  }, [range]);

  const total = useMemo(() => EMOTION_CATEGORIES.reduce((sum, c) => sum + (counts[c] || 0), 0), [counts]);
  const slices = useMemo(() => buildPie(counts), [counts]);

  const size = 260, R = 120, CX = size / 2, CY = size / 2;
      ...
      <View style={[styles.card, shadowCard]}>
        {total === 0 ? (
          ...
          <View style={styles.chartRow}>
            <Svg width={size} height={size}><G>
              {slices.map((s, i) => (
                <Path key={s.cat + i} d={arcPath(CX, CY, R, s.start - Math.PI / 2, s.end - Math.PI / 2)} fill={COLORS[s.cat]} />
              ))}
            </G></Svg>
            ...
            <View style={styles.legend}>
              ...
              {EMOTION_CATEGORIES.filter((c) => (counts[c] || 0) > 0).map((cat) => {
                const v = counts[cat] || 0; const pct = ((v / total) * 100).toFixed(0);
                return (
                  <View key={cat} style={styles.legendRow}>
                    <View style={[styles.legendChip, { backgroundColor: COLORS[cat] }]} />
                    <Text style={styles.legendLabel}>{cat.charAt(0).toUpperCase() + cat.slice(1)}</Text>
                    <Text style={styles.legendValue}>{v} • {pct}%</Text>
                  </View>
                );
              })}
            </View>
...
const styles = StyleSheet.create({
  ...
  rangeBtn: {
    ...
    borderWidth: StyleSheet.hairlineWidth, borderColor: colors.line, backgroundColor: colors.white,
  },
  rangeBtnActive: { borderColor: colors.text },
  rangeText: { color: colors.textSubtle },
  rangeTextActive: { fontWeight: "700", color: colors.text },

  card: {
    backgroundColor: colors.cream, borderRadius: 14, padding: 12,
    borderWidth: StyleSheet.hairlineWidth, borderColor: colors.line,
  },
  chartRow: { flexDirection: "row", alignItems: "center", gap: 12 },
  legend: { flex: 1 },
  total: { fontWeight: "700", marginBottom: 8, color: colors.text },
  legendRow: { flexDirection: "row", alignItems: "center", marginBottom: 6 },
  legendChip: { width: 14, height: 14, borderRadius: 7, marginRight: 8 },
  legendLabel: { flex: 1, color: colors.text },
  legendValue: { color: colors.textSubtle },

  empty: { textAlign: "center", color: colors.textSubtle, padding: 16 },
});
